...
def test_schema_stability_flags_gap_after_field_established():
    ...
    assert result["anomalies_sample"][0]["hypothesis"] is None


def test_schema_stability_classifies_traders_field_gap_as_review_required():
    """traders_*-Felder bekommen eine Hypothese (Schwellenwert-/
    Vertraulichkeitsregel), aber Status REVIEW_REQUIRED statt WARN -
    Befund und Hypothese bleiben explizit getrennt (Axel-Vorgabe)."""
    rows = [
    ...
    assert "suppression" in anomaly["hypothesis"]
