...
# ── Exchange Migration (Russell ICE→CME, 20.09.2026) ─────────────────────

def test_exchange_migration_not_applicable_for_other_instruments():
    result = cdq.check_exchange_migration([_row("2020-01-07", "2020-01-10")], "SP500")
    assert result["applicable"] is False


def test_exchange_migration_confirmed_when_sources_match_policy():
    """Vor transition_start nur ICE, nach transition_end nur CME, dazwischen
    beides (Overlap) - genau das erwartete Muster ergibt CONFIRMED."""
    rows = [
        # vor der Migration: nur ICE
        _row("2017-01-03", "2017-01-06", name="RUSSELL 2000 MINI INDEX FUTURE - ICE FUTURES U.S."),
        # im Uebergangsfenster: beide Quellen parallel (Overlap)
        _row("2017-09-12", "2017-09-15", name="RUSSELL 2000 MINI INDEX FUTURE - ICE FUTURES U.S.", historical_source_overlap=True),
        _row("2017-09-12", "2017-09-15", name="E-MINI RUSSELL 2000 INDEX - CHICAGO MERCANTILE EXCHANGE", historical_source_overlap=True),
        # nach der Migration: nur CME
        _row("2019-01-08", "2019-01-11", name="E-MINI RUSSELL 2000 INDEX - CHICAGO MERCANTILE EXCHANGE"),
    ]
    result = cdq.check_exchange_migration(rows, "RUSSELL2000")
    assert result["applicable"] is True
    m = result["migration"]
    assert m["status"] == "CONFIRMED"
    assert m["live_reverification"] == "CONFIRMED"
    assert m["research_policy"] == {"pre_migration": "ICE", "transition": "RESEARCH_ONLY", "post_migration": "CME"}
    assert len(m["evidence"]["internal"]) == 5
    assert m["evidence"]["external"]["source"].startswith("CME Group")
    # Keine Konsolidierungs-/Additions-Aussage darf im Ergebnis auftauchen:
    assert "combined" not in str(m).lower()
    assert "summed" not in str(m).lower()


def test_exchange_migration_flags_stale_if_ice_appears_after_transition_end():
    """Taucht nach transition_end ueberraschend wieder ICE auf (z.B. durch
    eine spaetere CFTC-Revision), MUSS das als STALE_NEEDS_REVIEW auffallen -
    keine blinde Weitergabe von CONFIRMED."""
    rows = [
        _row("2017-01-03", "2017-01-06", name="RUSSELL 2000 MINI INDEX FUTURE - ICE FUTURES U.S."),
        _row("2019-01-08", "2019-01-11", name="RUSSELL 2000 MINI INDEX FUTURE - ICE FUTURES U.S."),  # unerwartet nach transition_end
    ]
    result = cdq.check_exchange_migration(rows, "RUSSELL2000")
    assert result["migration"]["live_reverification"] == "STALE_NEEDS_REVIEW"


def test_exchange_migration_flags_stale_if_cme_appears_before_transition_start():
    rows = [
        _row("2016-01-05", "2016-01-08", name="E-MINI RUSSELL 2000 INDEX - CHICAGO MERCANTILE EXCHANGE"),  # unerwartet vor transition_start
    ]
    result = cdq.check_exchange_migration(rows, "RUSSELL2000")
    assert result["migration"]["live_reverification"] == "STALE_NEEDS_REVIEW"


def test_build_report_russell_migration_confirmed_unblocks_full_universe_research():
    """Kernfall: sobald die Migration CONFIRMED + live-reverifiziert ist,
    darf full_universe_research NICHT mehr allein wegen des Russell-Overlaps
    blockiert werden."""
    with tempfile.TemporaryDirectory() as tmpdir:
        rows = [
            _row("2017-01-03", "2017-01-06", name="RUSSELL 2000 MINI INDEX FUTURE - ICE FUTURES U.S."),
        ...
        _write_jsonl(tmpdir, "RUSSELL2000", rows)
        report = cdq.build_report(base_dir=tmpdir)

        # Overlap bleibt weiterhin diagnostisch dokumentiert:
        assert "RUSSELL2000" in report["checks"]["russell_overlap"]
        # Migration ist als eigener Befund vorhanden:
        assert "RUSSELL2000" in report["checks"]["exchange_migration"]
        assert report["checks"]["exchange_migration"]["RUSSELL2000"]["migration"]["status"] == "CONFIRMED"
        # full_universe_research darf jetzt NICHT mehr allein am Russell-Overlap scheitern:
        assert "Russell ICE/CME Overlap" not in report["research_readiness"]["reason"] or \
               "RUSSELL2000: Exchange-Migration" in report["research_readiness"]["reason"]
        notes = report["research_readiness"]["instrument_notes"]
        assert notes["RUSSELL2000"]["migration_status"] == "CONFIRMED"
        assert notes["RUSSELL2000"]["research_policy"]["transition"] == "RESEARCH_ONLY"


def test_build_report_russell_stale_migration_still_blocks():
    """Rueckfallposition: ist die Live-Reverifikation STALE, blockiert der
    Russell-Overlap full_universe_research weiterhin wie zuvor."""
    with tempfile.TemporaryDirectory() as tmpdir:
        rows = [
            _row("2016-01-05", "2016-01-08", name="E-MINI RUSSELL 2000 INDEX - CHICAGO MERCANTILE EXCHANGE"),  # widerspricht der Policy
            _row("2017-09-12", "2017-09-15", name="RUSSELL 2000 MINI INDEX FUTURE - ICE FUTURES U.S.", historical_source_overlap=True),
            _row("2017-09-12", "2017-09-15", name="E-MINI RUSSELL 2000 INDEX - CHICAGO MERCANTILE EXCHANGE", historical_source_overlap=True),
        ]
        _write_jsonl(tmpdir, "RUSSELL2000", rows)
        report = cdq.build_report(base_dir=tmpdir)
        assert report["checks"]["exchange_migration"]["RUSSELL2000"]["migration"]["live_reverification"] == "STALE_NEEDS_REVIEW"
        assert report["research_readiness"]["full_universe_research"] is False
