...
#
# AUSNAHME (18.09.2026): master["fairValueV2"] (s. compute_fair_value_v2()
# weiter unten) ist ABSICHTLICH NICHT Teil dieser Checkliste — es ist kein
# Feld auf results[]/scored[], sondern ein eigener, isolierter Top-Level-
# Key in master_market_data.json, bewusst außerhalb aller sieben Weiter-
# reichungsstellen gehalten (Fair-Value-Engine V2 ist PROVISIONAL/nicht
# kalibriert, s. docs/FAIR-VALUE-ENGINE-V2-SPEC-2026-09-17.md — darf noch
# in keinen Score/Leaderboard/Prompt/Digest einfließen).
...
# dokumentiert, s. dort. Reine Dokumentations-Ergaenzung, keine Code-
# Aenderung — Gewichtung von score_options_atmna() bewusst NICHT angepasst
# (Axel-Entscheidung nach Robustheitspruefung: niedrige Korrelation ist
# inhaltlich erwuenscht, kein Korrekturbedarf). weekly_income/collar
# bleiben unangetastet (Scope bewusst nicht wieder geoeffnet).
# v5.43.0 (16.09.2026): Backlog #64 — eigenstaendiger score_options_atmna(),
# nach Ontologie-Review-Entscheidung. Bisher teilte sich das options_atmna-
# Leaderboard den Score mit options_csp (top20("sCsp",50)) — bewusste
# Entscheidung vom 09.09.2026 ("Weg 1"), aber fuer die KANDIDATENBASIS
# gedacht (identische Screening-Kriterien EMA200/HVP/Earnings), nicht fuer
# die RANKING-Logik. Reviewer-Fund: score_options_csp() belohnt starkes
# Oversold (RSI<30, BB-Ausverkauf) massiv — passend fuer eine weit-OTM CSP
# (Delta 0.15-0.30 lt. ko-strategy-registry.js) mit viel Kurspuffer. Bei
# ATM/NA (Delta "ATM per Definition", kein Puffer lt. Registry) ist ein
# bereits stark ausverkaufter Titel STRUKTURELL RISKANTER, nicht attraktiver
# — Assignment ist schon wahrscheinlich, Management laeuft ueber aggressive
# Fruehausstiege/3-Stufen-Rollsystem statt Praemienpuffer.
#
# Fix, zweiteilig:
#   1. _options_base_candidate_gates() — gemeinsame Gates (Earnings/EMA200/
#      HVP) aus score_options_csp() extrahiert, jetzt von beiden Score-
#      Funktionen genutzt (verhindert Divergenz zwischen den beiden Basis-
#      Kriterien-Saetzen — dasselbe Prinzip wie bei ko-strategy-registry.js
#      gestern: eine Quelle statt zwei unabhaengig gepflegte).
#   2. score_options_atmna() — NEU: Neutralitaets-/Stabilitaetsbonus statt
#      Oversold-Bonus (RSI-Sweet-Spot 40-60 statt <30, BB-Sweet-Spot 30-70%
#      statt <=20%, zusaetzlicher tightnessPct-Stabilitaetsbonus). Bewusst
#      KEINE reine Score-Umkehrung (Reviewer-Einwand: haette ueberkaufte
#      Titel bevorzugt, zu aggressiv) — Regime/HVP-Gewichtung identisch zu
#      CSP uebernommen, nur RSI/BB neu gewichtet.
#   weekly_income bleibt BEWUSST bei sCsp (kurzfristige CSP-Variante,
#   dieselbe Short-Put-Logik, kein konkreter Grund fuer eigene Ranking-
#   Logik). collar bleibt VORERST ebenfalls bei sCsp, aber explizit als
#   dokumentierte Wissensluecke markiert (Axel-Einwand): Collar ist
#   strukturell Long Stock + Protective Put + Covered Call, keine Short-
#   Put-Position — sCsp beantwortet moeglicherweise die falsche Frage.
#   Bewusst NICHT jetzt score_options_collar() gebaut (Scope-Disziplin).
#
#   VERIFIZIERT: score_options_csp() nach Refactor gegen 2000 randomisierte
#   Testfaelle byte-identisch zum Vorher-Stand (0 Abweichungen) — die
#   Reviewer-Vorgabe "score_options_csp() bleibt unveraendert" ist damit
#   nicht nur behauptet, sondern getestet. score_options_atmna() gegen drei
#   Beispielszenarien plausibilisiert (Extrem-Oversold: CSP=100/ATMNA=45;
#   Neutral+eng: CSP=50/ATMNA=100; Ueberkauft: CSP=30/ATMNA=5) — noch NICHT
#   gegen echte Live-Daten verifiziert, das zeigt erst der naechste Lauf.
# v5.42.3 (15.09.2026): dritte Fundstelle desselben alten Fence-Stripping-
# Bug-Patterns behoben — Master-Shortlist-KI-Enrichment (die "KI {sym}:
# Trigger=... SL=... CRV=..."-Zeilen) nutzte noch die alte, fragile Logik
# (`if text.startswith("```"): text.split('\n')[1:-1]`). Diese Stelle
# zeigte heute KEINE Live-Fehler (Prompt hat bereits "Antworte NUR mit dem
# JSON-Objekt — kein Markdown, kein Praeambel" — dieselbe starke Anweisung,
# die beim ersten Options-KI-Call auch fehlerfrei lief), aber derselbe
# strukturelle Bug war latent vorhanden. Fix: _extract_json_object() dafuer
# UND fuer enrich_options_watchlist_with_ai() gemeinsam genutzt — dazu von
# einer lokal verschachtelten Funktion auf Modulebene gehoben (vorher: in
# enrich_options_watchlist_with_ai() verschachtelt, nur dort aufrufbar).
# Vermeidet eine dritte Kopie desselben Codes (genau das Kopie-Drift-Muster,
# das in diesem Projekt bereits mehrfach dokumentiert Probleme verursacht
# hat). Gleiches Diagnose-Logging (roher Text bei Parse-Fehler) ergaenzt,
# konsistent mit den beiden anderen Call-Sites. max_tokens (400) und Prompt
# unveraendert gelassen — kein akuter Fehlerbefund, der das rechtfertigt.
# v5.42.2 (15.09.2026): Options-KI EIC-Layer — ZWEITE Iteration nach echtem
# Rohtext (Diagnose-Log aus v5.42.1 direkt live ausgewertet, s.u.). Die
# v5.42.1-Hypothese ("nur eine ueberschuessige Fence-Zeile") war FALSCH.
# Echter Rohtext zeigte zwei Muster, beide ausschliesslich beim zweiten
# Call (strike_prompt/EIC-Layer, NIE beim ersten): (a) valides JSON gefolgt
# von Fence + mehrzeiliger Herleitung (ACAD/PYPL/SHW/MFG/TRP/VICI) — die
# alte $-verankerte Fence-Regex griff nicht, da die Fence nicht am Textende
# stand; (b) Modell schreibt ausfuehrliche Herleitung VOR dem JSON, das
# knappe max_tokens=150-Budget war dann oft aufgebraucht, BEVOR ueberhaupt
# JSON generiert wurde (UNP/NSC/PLD/ADBE/ODFL/ENGIY/XLRE) — kein JSON im
# Response-Text auffindbar. Root Cause fuer beide: strike_prompt hatte,
# anders als der erste (fehlerfreie) Options-KI-Prompt, KEINE explizite
# "kein Markdown/keine Erlaeuterung"-Anweisung. Fix, zweiteilig:
#   1. strike_prompt um "Antworte AUSSCHLIESSLICH mit dem JSON-Objekt
#      selbst. Keine Herleitung, keine Erlaeuterung, kein Markdown, keine
#      Code-Fence, kein Text davor oder danach." ergaenzt (Root-Cause-Fix,
#      adressiert primaer Muster (b), spart nebenbei Tokens/Kosten, da das
#      Modell keine unerwuenschte Prosa mehr generiert).
#   2. _strip_json_fence() durch _extract_json_object() ersetzt (Klammer-
#      Extraktion statt Fence-Heuristik) — findet das erste vollstaendige,
#      klammerbalancierte JSON-Objekt unabhaengig von Position/Umgebung,
#      robuste Absicherung fuer Muster (a) und jede Kombination aus Prosa
#      vor/nach dem JSON. Gegen alle vier Muster-(a)-Rohtexte von heute
#      (PYPL/SHW/MFG/VICI) lokal getestet, alle vier parsen jetzt korrekt.
#   Muster (b) NICHT vorab testbar (kein JSON im geloggten Text vorhanden) —
#   ob der Prompt-Fix dafuer ausreicht, zeigt erst der naechste Live-Lauf.
#   max_tokens=150 bewusst NICHT erhoeht (kostenbewusst): bei entfernter
#   Prosa ist das Budget fuer das kompakte 4-Feld-JSON deutlich ausreichend.
...
def _options_base_candidate_gates(r: dict) -> tuple:
    """
    Gemeinsame Gates fuer alle CSP-Varianten (csp_wheel/atmna/weekly_income/
    collar) — Earnings-Gate, EMA200-Gate, HVP-Gate. NEU (16.09.2026, Backlog
    #64, Ontologie-Review-Entscheidung): extrahiert aus score_options_csp(),
    damit score_options_atmna() dieselben Titelauswahl-Kriterien nutzt, ohne
    sie unabhaengig zu pflegen (Divergenz-Risiko sonst identisch zum
    5-Feldlisten-Fund vom 07.09.2026). Gemeinsame Kandidatenbasis-
    Entscheidung vom 09.09.2026 ("Weg 1") bleibt unveraendert — nur die
    RANKING-Logik INNERHALB dieser Basis unterscheidet sich jetzt je nach
    Positionsstruktur (OTM-Puffer vs. ATM-ohne-Puffer).

    Returns: (blocked: bool, eg_sev: str|None)
    Bei blocked=True soll die aufrufende Score-Funktion sofort 0
    zurueckgeben, ohne eg_sev weiter zu verwenden.
    """
    ...
    if not ema200:
        return True, None
    if price < ema200 * 0.85:   # Gemini: war 0.95 → 0.85
        return True, None
    ...
    return False, _eg_sev


def score_options_csp(r: dict) -> int:
    """
    Cash-Secured Put Score 0-100 — Unleashed v2 (Gemini-Blueprint).
    Deckt Wheel und wöchentliche CSP+LongCall-Kombinationen ab (NICHT mehr
    ATM-CSP — s. score_options_atmna(), 16.09.2026, eigene Score-Logik).
    ...
    v5.43 (16.09.2026): Gates nach _options_base_candidate_gates() ausgelagert
    (identisches Verhalten, nur intern refaktoriert — regressionsgetestet
    gegen 50+ synthetische Faelle, byte-identische Scores vor/nach Refactor).
    """
    blocked, eg_sev = _options_base_candidate_gates(r)
    if blocked:
        return 0
...
def score_options_atmna(r: dict) -> int:
    """
    ATM/NA (At-The-Money, No Adjustment) Score 0-100 — NEU (16.09.2026,
    Backlog #64, Ontologie-Review-Entscheidung).

    ═══ VALIDIERUNGS-BASELINE (16.09.2026, master_market_data.json vom
    16.09.2026 07:24, 733 Ticker, Gate-Population 450) — NICHT als
    Zielwerte misszuverstehen, sondern als Referenzpunkt: falls eine
    kuenftige Aenderung an score_options_csp()/score_options_atmna() diese
    Struktur deutlich verschiebt (insbesondere: Top-20/50-Overlap wieder
    > 0%, oder die monoton wachsende Overlap-Kurve verschwindet), war das
    vermutlich unbeabsichtigt — die beiden Scores sollten bewusst
    unterschiedliche Praeferenzen im selben Kandidatenuniversum abbilden,
    keine leicht variierten Versionen desselben Rankings. ═══
        Top-20 Overlap:   0%
        Top-50 Overlap:   0%
        Top-100 Overlap: 14%
        Top-200 Overlap: 40%
        Spearman rho (gesamte Gate-Population): 0.0957 (p=0.042)
        Pearson r:                              0.108
    Erwartete Struktur, nicht Zielwerte: starke Trennung an den
    Extremraendern (CSP sucht Oversold/Puffer, ATMNA sucht Neutralitaet/
    Stabilitaet), wachsende Ueberlappung mit zunehmender Rangbreite,
    identische Gate-Population. Herleitung/Diskussion im Chat-Verlauf
    16.09.2026 (Ontologie-Review-Session), nicht in einer separaten Datei.

    Nutzt dieselben Gates wie score_options_csp() (_options_base_candidate_
    gates()) — die Titelauswahl-Basis (EMA200/HVP/Earnings) ist fuer alle
    CSP-Varianten identisch (Entscheidung 09.09.2026, "Weg 1", unveraendert).

    ABER: score_options_csp() belohnt starkes Oversold (RSI<30, BB-Ausverkauf
    am unteren Band) massiv — sinnvoll fuer eine WEIT AUS DEM GELD liegende
    CSP (Delta 0.15-0.30 laut ko-strategy-registry.js csp_wheel.rules): viel
    Puffer zum Strike, ein weiter fallender Kurs bedroht die Position noch
    nicht sofort. Bei ATM/NA liegt der Strike laut Registry "ATM per
    Definition" (deltaRange: null, kein Delta-Fenster) — kein Puffer. Ein
    bereits stark ausverkaufter Titel ist hier riskanter statt attraktiver:
    Assignment ist strukturell schon wahrscheinlich, das Management laeuft
    ueber aggressive Fruehausstiege (Registry: profitTaking 50-70% je nach
    Rest-DTE) und ein 3-stufiges Rollsystem, nicht ueber Praemien-Puffer.

    Score-Semantik (Reviewer-Entscheidung 16.09.2026): "Wie geeignet ist
    dieser Titel fuer einen ATM-Put, bei dem Stabilitaet und kontrolliertes
    Management wichtiger sind als maximaler Abstand zum Strike?" — bewusst
    KEINE reine Umkehrung des CSP-Scores (waere zu aggressiv, wuerde bereits
    ueberkaufte Titel bevorzugen), sondern ein Neutralitaets-/Stabilitaets-
    Bonus fuer die mittlere Zone, mit mildem statt hartem Malus an den
    Raendern.

    weekly_income bleibt BEWUSST bei score_options_csp() (Reviewer-
    Entscheidung: keine dritte Ranking-Logik ohne konkreten strategischen
    Grund — anders als bei ATM/NA gibt es fuer Weekly-Puts, laut Registry
    mit Delta-Bandbreite 0.25-0.50, keinen ebenso klaren Puffer-Unterschied
    zu CSP, der eine eigene Score-Logik rechtfertigen wuerde).

    collar bleibt ebenfalls vorerst bei score_options_csp() — ABER das ist
    eine dokumentierte Wissensluecke, kein Fall wie weekly_income (Axel-
    Einwand 16.09.2026): Ein Collar ist strukturell Long Stock + Protective
    Put + Covered Call, keine Short-Put-Position wie CSP — sCsp beantwortet
    hier moeglicherweise die falsche Frage. S. Backlog-Kommentar bei der
    options_collar-Leaderboard-Zeile.
    """
    blocked, eg_sev = _options_base_candidate_gates(r)
    if blocked:
        return 0
    ...
    bbpos     = r.get("bbPos")
    regime    = (r.get("regime") or "").lower()
    tightness = r.get("tightnessPct")   # 5T-Range% aus calc_vcp(), hier als
                                         # Stabilitaetsproxy zweckentfremdet

    s = 0

    # RSI: neutrale Zone (40-60) bevorzugt statt Extrem-Oversold — s. Docstring.
    if   rsi < 30:  s -= 10   # zu weit ausverkauft fuer ATM (kein Puffer)
    elif rsi < 40:  s += 5    # leicht positiv
    elif rsi <= 60: s += 30   # Sweet Spot fuer ATM/NA
    elif rsi <= 70: s += 0    # neutral
    else:           s -= 15  # überkauft

    # Bollinger-Position: mittlere Zone bevorzugt statt Ausverkaufszone am
    # unteren Band (dort punktet score_options_csp() maximal, hier bewusst
    # nicht).
    if bbpos is not None:
        if   bbpos <= 0.20: s += 0    # kein Bonus (anders als CSP: dort +30)
        elif bbpos <= 0.70: s += 20   # Sweet Spot
        elif bbpos <= 0.80: s += 0    # neutral
        else:               s -= 10  # überkauft

    # Regime: identisch zu CSP — Seitwaertsphasen bleiben Paradedisziplin,
    # unabhaengig von OTM-Puffer oder ATM (Reviewer-Tabelle: "wichtig" fuer
    # beide, keine Differenzierung vorgegeben).
    if   regime == "side":     s += 30
    elif regime == "bull":     s += 20
    elif regime == "volatile": s += 10

    # HVP-Bonus: identisch zu CSP-Gewichtung (Reviewer-Tabelle: "wichtig"
    # fuer beide, keine Differenzierung vorgegeben).
    s += min(hvp // 5, 15)

    # NEU ggü. CSP: Stabilitaets-Bonus ueber tightnessPct (bereits etablierte
    # 5T-Range%-Metrik aus calc_vcp()/VCP-Scoring, <3%=Tight/<5%=OK) — enge
    # juengste Handelsspanne passt strukturell besser zu ATM (kein Puffer
    # noetig, wenn der Kurs ohnehin wenig schwankt) als ein bereits laufender
    # Ausverkauf.
    if tightness is not None:
        if   tightness < 3: s += 15
        elif tightness < 5: s += 8

    # Earnings-Malus (weich, 8-14T): identisch zu CSP.
    if eg_sev == "soft":
        s = max(0, s - 20)
...
# ── FAIR-VALUE-ENGINE V2 — MECHANIK, KEINE KALIBRIERUNG (18.09.2026) ──────
# Spec: docs/FAIR-VALUE-ENGINE-V2-SPEC-2026-09-17.md, UIQ-Suite (eingefroren,
# 17.09.2026). Diese Spec verbietet Produktionscode/Parameter-Kalibrierung,
# solange r30/r90-Historie und ownerEarningsYield-Historie (Feld existiert
# erst seit 09.09.2026) zu duenn sind — beides Stand 18.09.2026 weiterhin
# der Fall. Reviewer-Praezisierung (18.09.2026, von Axel geteilt), die
# diesen Block trotzdem erlaubt: die Sperre gilt fuer die EMPIRISCHE
# KALIBRIERUNG (welche Multiples/Gewichte/Schwellen tatsaechlich stimmen),
# NICHT fuer die MECHANIK (Berechnungslogik/Datenmodell) selbst, solange
# keine empirisch behaupteten Parameter festgezurrt werden. Axel-Entscheidung
# (18.09.2026): "Go fuer die technische Implementierung ... No-Go fuer
# Kalibrierung, Optimierung und produktive Verwendung des Fair-Value-Signals."
#
# ISOLATIONS-GARANTIE (wichtigster Punkt, s. Reviewer-Punkt "eigene, saubere
# Trennung"): das Ergebnis landet NICHT in results[]/scored[]/den
# Leaderboards (anders als alle anderen Fundamentalfelder oben) — dadurch
# gilt die SIEBEN-STELLEN-CHECKLISTE am Dateikopf HIER NICHT, und es ist
# strukturell ausgeschlossen, dass ein Score, ein Leaderboard, ein KI-Prompt
# (ko-prompts.js) oder der Public Digest (generate_public_recommendations.js)
# versehentlich mitliest — keiner dieser Konsumenten kennt den Schluessel
# "fairValueV2", er muesste explizit neu angebunden werden. Stattdessen ein
# EIGENER, separater Top-Level-Key in master_market_data.json
# (master["fairValueV2"], s. Aufrufstelle nach dem Fundamental-Enrichment-
# Block weiter unten in main()) — exakt die im Reviewer-Vorschlag geforderte
# Trennung von "Mechanik bauen" und "produktiv verwenden".
#
# calibration_status ist ein PFLICHTFELD auf jedem einzelnen Ergebnis (nicht
# nur einmal global dokumentiert) — damit bleibt die Provisorik auch dann
# sichtbar, wenn jemand (Mensch oder KI) in drei Monaten nur einen einzelnen
# Datensatz isoliert betrachtet, ohne diesen Kommentarblock zu lesen.
FAIR_VALUE_V2_CALIBRATION_STATUS = "PROVISIONAL_NOT_CALIBRATED"
FAIR_VALUE_V2_MODEL_NAME    = "yield_reframing_v2"
FAIR_VALUE_V2_MODEL_VERSION = "2.0"

# UMBENANNT (18.09.2026, Reviewer-Feedback): "Benchmark" suggeriert bereits
# sprachlich, diese Werte seien empirisch validiert — sind sie ausdruecklich
# NICHT. Jetzt "PROVISIONAL_REFERENCE_YIELDS" genannt (grobe, oeffentlich
# gaengige Value-Investing-Faustregeln, damit die Mechanik ueberhaupt einen
# Bezugspunkt hat, KEINE UIQ-eigene empirische Kalibrierung):
#   - oe/fcf: 5.0% entspricht einem impliziten Multiple von 20x auf Owner
#     Earnings bzw. Free Cashflow — haeufig zitierte "fair bewertet"-
#     Schwelle in Value-Screening-Literatur, keine UIQ-Herleitung.
#   - pe: 1/18 ≈ 5,56% entspricht einem Forward-KGV von ca. 18 (grober,
#     langjaehriger Marktdurchschnitt), ebenfalls keine UIQ-Herleitung.
# ABSICHTLICH extern/zentral als Konstanten gehalten (Reviewer-Vorschlag:
# "alle spaeter kalibrierbaren Parameter extern konfigurierbar"), damit die
# spaetere Kalibrierungsphase (Spec Phase D) NUR diese Werte ersetzen muss,
# nicht die Berechnungslogik selbst.
FAIR_VALUE_V2_PROVISIONAL_REFERENCE_YIELDS_PCT = {
    "oe":  5.0,
    "fcf": 5.0,
    "pe":  round(100.0 / 18.0, 2),
}

# Dispersion-Schwellen fuer fair_value_agreement (ebenfalls PROVISORISCH,
# reine deskriptive Bucketing-Grenzen, keine empirische Kalibrierung — s.
# Reviewer-Beispiel: OE=$180/FCF=$190/PE=$410 -> Dispersion=121% -> klar
# "nicht einig", auch bei data_completeness=A).
FAIR_VALUE_V2_AGREEMENT_THRESHOLDS_PCT = {"high_max": 15.0, "medium_max": 40.0}


def compute_fair_value_v2(sym: str, price: float, fund_data: dict) -> dict:
    """
    Fair-Value-Engine V2 — MECHANIK (18.09.2026). Reine Berechnungslogik,
    KEINE empirisch behaupteten Parameter (s. Modul-Kommentar oben).

    Reframing aus der Spec: alle drei Anker werden als Renditegroessen
    behandelt (Owner-Earnings-Yield, FCF-Yield, Forward-Earnings-Yield =
    100/peForward), nicht als drei getrennte Bewertungsformeln — mathema-
    tisch aequivalent, s. Spec-Abschnitt "Reframing". Je Anker:
        fair_value_anchor = price * (yield_actual / referenz_yield)
    Diese Form ist fuer alle drei Anker identisch (kein Anker bekommt eine
    strukturell andere Formel), nur die drei Eingaben (yield_actual,
    referenz_yield aus FAIR_VALUE_V2_PROVISIONAL_REFERENCE_YIELDS_PCT)
    unterscheiden sich. "Referenz", NICHT "Benchmark" — s. Konstanten-
    Kommentar oben, bewusste Terminologie-Korrektur (18.09.2026, Reviewer).

    fair_value_model/-_model_version (18.09.2026, Reviewer-Ergaenzung):
    macht spaeter, wenn Referenzwerte/Methodik sich aendern, nachvollzieh-
    bar, mit welcher Engine-Version ein historischer Fair Value erzeugt
    wurde — wichtig fuer Track-Record-Vergleichbarkeit ueber Zeit.

    fair_value/-_low/-_high: NICHT gewichtet, NICHT optimiert (Spec-
    Grundsatz "kein Optimieren auf ρ") — schlicht Median/Min/Max der
    verfuegbaren Anker-Werte. Bei nur einem verfuegbaren Anker ist
    fair_value_dispersion_pct/fair_value_agreement bewusst None/"N/A"
    (Dispersion braucht mindestens zwei Werte).

    Rueckgabe ist bewusst REIN DESKRIPTIV — kein Ranking-Score, kein Gate,
    keine BUY/HOLD/SELL-Ableitung (Spec-Verbot). `fair_value_upside_pct`
    ist Metadatum, kein Handelssignal.
    """
    anchors = {}  # anchor_name -> (yield_actual_pct, fair_value)

    oe_yield = fund_data.get("ownerEarningsYield")
    if oe_yield is not None and price:
        anchors["oe"] = (oe_yield, price * (oe_yield / FAIR_VALUE_V2_PROVISIONAL_REFERENCE_YIELDS_PCT["oe"]))

    fcf_yield = fund_data.get("fcfYield")
    if fcf_yield is not None and price:
        anchors["fcf"] = (fcf_yield, price * (fcf_yield / FAIR_VALUE_V2_PROVISIONAL_REFERENCE_YIELDS_PCT["fcf"]))

    pe_fwd = fund_data.get("peForward")
    if pe_fwd is not None and pe_fwd > 0 and price:
        pe_yield = round(100.0 / pe_fwd, 3)
        anchors["pe"] = (pe_yield, price * (pe_yield / FAIR_VALUE_V2_PROVISIONAL_REFERENCE_YIELDS_PCT["pe"]))

    n = len(anchors)
    completeness = {0: "N/A", 1: "C", 2: "B", 3: "A"}[n]
    ...
    if n == 0:
        return {
            ...
            "fair_value_dispersion_pct": None,
            "fair_value_data_completeness": completeness,
            "fair_value_agreement": "N/A",
            "valuation_method": [],
            "valuation_inputs": {},
            "fair_value_model": FAIR_VALUE_V2_MODEL_NAME,
            "fair_value_model_version": FAIR_VALUE_V2_MODEL_VERSION,
            "calibration_status": FAIR_VALUE_V2_CALIBRATION_STATUS,
            "valuation_timestamp": datetime.now(timezone.utc).isoformat(),
        }

    values = sorted(v for (_y, v) in anchors.values())
    fv_low, fv_high = values[0], values[-1]
    # Median statt gewichtetem Mittel — s. Docstring, kein implizites Gewicht.
    if n % 2 == 1:
        fv = values[n // 2]
    else:
        fv = (values[n // 2 - 1] + values[n // 2]) / 2.0

    if n >= 2 and fv != 0:
        dispersion_pct = round((fv_high - fv_low) / abs(fv) * 100.0, 1)
        thr = FAIR_VALUE_V2_AGREEMENT_THRESHOLDS_PCT
        if dispersion_pct <= thr["high_max"]:
            agreement = "HIGH"
        elif dispersion_pct <= thr["medium_max"]:
            agreement = "MEDIUM"
        else:
            agreement = "LOW"
    else:
    ...
    upside_pct = round((fv / price - 1) * 100.0, 1) if price else None
    ...
    return {
        ...
        "fair_value_upside_pct": upside_pct,
        "fair_value_dispersion_pct": dispersion_pct,
        "fair_value_data_completeness": completeness,
        "fair_value_agreement": agreement,
        "valuation_method": sorted(anchors.keys()),
        "valuation_inputs": {
            k: {"yield_pct": y, "fair_value_anchor": round(v, 2)}
...
def compute_fair_value_v2_for_universe(fund_cache: dict, results: list) -> dict:
    """
    Treiber ueber alle Kandidaten, fuer die Fundamentaldaten vorliegen
    (dieselbe Menge wie das bestehende Fundamental-Enrichment, s.
    Aufrufstelle in main()). Preis kommt aus results[] (einzige
    Kursquelle, konsistent mit dem Rest des Aggregators).
    """
    price_by_sym = {r["sym"]: r.get("price") for r in results if r.get("sym")}
    out = {}
    for sym, fund_data in fund_cache.items():
        price = price_by_sym.get(sym)
        if not price:
            continue
        out[sym] = compute_fair_value_v2(sym, price, fund_data)
    return out
...
def build_leaderboards(results: list, market_regime: str = "NEUTRAL") -> dict:
    ...
    for r in results:
        ...
        # NEU (16.09.2026, Backlog #64, Ontologie-Review-Entscheidung): eigener
        # ATM/NA-Score statt Wiederverwendung von s_csp — s. Docstring bei
        # score_options_atmna(). Titelauswahl-Basis bleibt identisch (Weg 1,
        # 09.09.2026), nur die Ranking-Logik unterscheidet sich jetzt.
        s_atmna     = score_options_atmna(r)
        # NEU (14.09.2026, UIQ Spec v1.2 §2.1): Underlying Assignment Quality —
        ...
        scored.append({
            ...
            "sFading":       s_fading,
            "sCsp":          s_csp,
            "sAtmna":        s_atmna,   # NEU (16.09.2026, Backlog #64)
            "sCc":           s_cc,
            "sVcp":          s_vcp,
    ...
    leaderboards = {
        ...
        #
        # PRAEZISIERT (16.09.2026, Backlog #64, Ontologie-Review): Die
        # Kandidatenbasis-Entscheidung von oben gilt weiterhin — ABER
        # gleiche Kandidatenbasis heisst nicht zwingend gleicher Ranking-
        # Score. atmna nutzt jetzt score_options_atmna() (eigene Ranking-
        # Logik: Neutralitaets-/Stabilitaetsbonus statt Oversold-Bonus, s.
        # dortiger Docstring).
        #
        # weekly_income bleibt bewusst bei sCsp — kurzfristige CSP-Variante,
        # dieselbe Positionslogik (Short Put) wie csp_wheel, nur kuerzere
        # DTE. Kein konkreter strategischer Grund fuer eine eigene Ranking-
        # Logik.
        #
        # collar bleibt vorerst EBENFALLS bei sCsp — aber das ist eine
        # bewusst dokumentierte Wissensluecke, kein "genauso okay wie
        # weekly_income"-Fall (Axel-Einwand 16.09.2026): Ein Collar ist
        # strukturell Long Stock + Protective Put + Covered Call, keine
        # Short-Put-Position — die Frage "eignet sich dieser Titel fuer
        # einen OTM-CSP-Entry" (was sCsp beantwortet) ist fuer einen Collar
        # nicht die richtige Frage. sCsp ist hier moeglicherweise semantisch
        # unpassend, nicht nur "ausreichend generisch" wie bei weekly_income.
        # BACKLOG (kein neuer Ticket-Bezeichner vergeben, s. UEBERGABE-
        # 2026-09-16.md): collar shares the generic options candidate score
...
def _extract_json_object(raw: str) -> str:
    """Extrahiert das erste vollstaendige, klammerbalancierte JSON-Objekt
    aus einem Text — unabhaengig davon, ob davor/danach Erklaertext,
    Markdown-Ueberschriften oder Code-Fences stehen.

    Modulebene seit 15.09.2026 (zuvor lokal verschachtelt in
    enrich_options_watchlist_with_ai()) — wird jetzt von ZWEI KI-Enrichment-
    Funktionen genutzt (Options-Watchlist UND Master-Shortlist, beide mit
    demselben alten, fragilen Fence-Stripping-Bug); auf Modulebene gehoben,
    um genau die Art von Kopie-Drift zu vermeiden, die in diesem Projekt
    bereits mehrfach dokumentiert Probleme verursacht hat (s. z.B. die
    "5 unabhaengige Feldlisten"-Historie vom 07.09.2026).

    ZWEITE Iteration, 15.09.2026 (die erste Fassung — reine Fence-
    Entfernung, s. AGGREGATOR_VERSION-Changelog — ging von einer falschen
    Hypothese aus: nur eine ueberschuessige Fence-ZEILE, kein Erklaertext
    drumherum). Echter Rohtext (Diagnose-Log derselben Version, direkt
    live geprueft) zeigt zwei tatsaechliche Muster, beide vom
    strike_prompt-Call (EIC-Layer), der anders als der erste Options-KI-
    Prompt KEINE "kein Markdown/keine Erlaeuterung"-Anweisung hatte:
    (a) valides JSON, gefolgt von einer Fence-Zeile UND anschliessender
    mehrzeiliger Herleitung/Tabelle (z.B. PYPL/SHW/MFG/TRP/ACAD) —
    "Extra data" bei der alten Logik, da die Fence nicht am Textende
    stand; (b) das Modell beginnt mit ausfuehrlicher Herleitung VOR dem
    JSON, das knappe max_tokens=150-Budget des zweiten Calls ist dann
    oft aufgebraucht, bevor ueberhaupt JSON generiert wird (z.B. UNP/
    NSC/PLD/ADBE/ODFL/ENGIY/XLRE) — "Expecting value" bei der alten
    Logik. Primaerer Fix dafuer ist eine verschaerfte Prompt-Anweisung
    im strike_prompt (s. dort); diese Funktion ist die zusaetzliche,
    Prompt-unabhaengige Absicherung fuer Fall (a) und alle Ticker, bei
    denen die Klammer trotzdem vorhanden ist. LIVE VERIFIZIERT (15.09.2026,
    18:11-18:14 Uhr Lauf): Options-Watchlist-Fehlerquote 0/15.
    """
    start = raw.find('{')
    if start == -1:
        return raw.strip()  # kein JSON auffindbar — json.loads() soll den echten Fehler werfen
    depth = 0
    in_string = False
    escape = False
    ...
    return raw[start:].strip()  # unvollstaendig (echte Trunkierung) — json.loads() soll failen
...
Antworte AUSSCHLIESSLICH mit dem JSON-Objekt selbst. Keine Herleitung, keine
Erläuterung, kein Markdown, keine Code-Fence, kein Text davor oder danach."""
                ...
                with urllib.request.urlopen(req2, timeout=15) as resp2:
                    ...
                    text2 = _extract_json_object(text2)
                    try:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as resp:
                ...
                try:
                    ...
                    # Diagnose-Logging analog zu enrich_options_watchlist_with_ai()
                    # (15.09.2026) — dieselbe Klammer-Extraktion, hier bisher ohne
                    # beobachtete Fehler live, aber derselbe alte Bug-Pattern war
                    # vorher auch hier vorhanden (Fence-Stripping nur bei
                    # oeffnender Fence). Falls doch mal was durchrutscht, gleich
                    # den echten Text statt einer neuen Hypothese.
                    log.warning(f"    KI {sym} RAW-Response nach Parse-Fehler: {text[:500]!r}")
                    raise
...
def main():
    ...
    if _fund_candidates:
        ...
        # ── FAIR-VALUE-ENGINE V2 (18.09.2026, Mechanik, s. Modul-Kommentar bei
        # compute_fair_value_v2()) — bewusst NUR hier, NICHT in results[]/
        # scored[]/Leaderboards geschrieben. Eigener Top-Level-Key
        # "fairValueV2" in master (s. Zuweisung unten, master ist zu diesem
        # Zeitpunkt bereits als Dict angelegt) — kein Fehlschlag hier darf
        # den Hauptlauf gefaehrden (§4-Grundsatz wie beim Rest des
        # Enrichment-Blocks), daher eigener try/except.
        try:
            _fair_value_v2 = compute_fair_value_v2_for_universe(_fund_cache, results)
            master["fairValueV2"] = _fair_value_v2
            log.info(f"  ✅ Fair-Value-Engine V2 (PROVISIONAL, unkalibriert): "
                     f"{len(_fair_value_v2)}/{len(_fund_cache)} Ticker berechnet")
        except Exception as _fv2_err:
            log.warning(f"  ⚠ Fair-Value-Engine V2 fehlgeschlagen: {_fv2_err}")
            master["fairValueV2"] = {}
    else:
