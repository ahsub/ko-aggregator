...
# Bestaetigte Exchange-Migrationen (20.09.2026, Axel + Reviewer + Claude):
# im Unterschied zu KNOWN_CROSS_VALIDATED_GAPS geht es hier nicht um eine
# Datenluecke, sondern um einen realen, dauerhaften Boersenwechsel - beide
# Quellen sind fuer ihre jeweilige Zeitphase korrekt, nicht "eine richtiger
# als die andere". KEINE nachtraegliche Addition von ICE+CME (das waere
# eine eigene, unbelegte fachliche Annahme ueber Doppelzaehlung/Netting) und
# KEINE Entfernung der 43 Overlap-Wochen aus dem Coverage-/Overlap-Befund -
# sie bleiben dort als erklaertes historisches Uebergangsereignis stehen,
# dieser Abschnitt kommt zusaetzlich dazu.
#
# RUSSELL2000-Fall: CME fuehrte im Juli 2017 eigene Russell-2000-Futures ein;
# nach dem ersten Quartals-Rollover (September- auf Dezember-Kontrakt) ging
# It. CME Group offizieller Pressemitteilung >91% des zuvor bei ICE
# gehaltenen Open Interest zu CME ueber. Intern bestaetigt durch das exakt
# gleiche Migrationsmuster in Open Interest UND allen vier TFF-Kategorien
# (Dealer/Asset Manager/Leveraged Funds/Other Reportables) - ICE-Werte
# laufen ab 2017-09-12 gegen Null, CME uebernimmt spiegelbildlich die
# Groessenordnung. transition_start/-end sind der dokumentierte Zeitraum
# der Umstellung (CME-Angabe: Einfuehrung Juli 2017; Uebergangsabschluss
# konservativ auf das Ende unseres beobachteten Overlap-Fensters gelegt,
# 2018-06-05) - NICHT identisch mit dem Beginn unserer eigenen Datenreihe
# (2017-08-15, s. KNOWN_SOURCE_OVERLAPS), die beiden Daten haben bewusst
# unterschiedliche Bedeutung und werden hier nicht vermischt.
KNOWN_EXCHANGE_MIGRATIONS = {
    "RUSSELL2000": {
        "status": "CONFIRMED",
        "from_exchange": "ICE",
        "to_exchange": "CME",
        "transition_start": "2017-07-01",
        "transition_end": "2018-06-05",
        "research_policy": {
            "pre_migration": "ICE",
            "transition": "RESEARCH_ONLY",
            "post_migration": "CME",
        },
        "evidence": {
            "internal": [
                "open_interest_migration",
                "dealer_net_position",
                "asset_manager_net_position",
                "leveraged_funds_net_position",
                "other_reportables_net_position",
            ],
            "external": {
                "source": "CME Group press release, 03.10.2017",
                "source_url": "https://www.cmegroup.com/media-room/press-releases/2017/10/03/cme_group_reachedaveragedailyvolumeof17millioncontractsinseptemb.html",
                "claim": (
                    "Following CME Group's launch of Russell 2000 futures in "
                    "July, and first quarterly rollover of positions from "
                    "September quarterly contract to December quarterly "
                    "contract, more than 91 percent of open interest "
                    "previously held on Intercontinental Exchange (ICE) "
                    "transitioned to CME Group"
                ),
                "note": (
                    "Externe Primaerquellen-Bestaetigung - KEIN aus UIQ-Daten "
                    "berechnetes Ergebnis. Die 91%-Zahl stammt direkt von "
                    "CME Group, nicht aus unserer eigenen Open-Interest-Reihe."
                ),
            },
        },
...
def check_exchange_migration(rows: list, instrument_id: str) -> dict:
    """
    Reicht den dokumentierten Migrationsbefund aus KNOWN_EXCHANGE_MIGRATIONS
    durch, mit einer Live-Konsistenzpruefung gegen die aktuellen Daten
    (analog zu check_cross_validated_gaps() - keine blinde Weitergabe eines
    einmal dokumentierten Befunds):
    (a) vor transition_start duerfen nur Zeilen der from_exchange-Quelle
        vorkommen (per Namens-Substring, z.B. "ICE"),
    (b) nach transition_end duerfen nur Zeilen der to_exchange-Quelle
        vorkommen (z.B. "CME"/"CHICAGO"),
    (c) im Uebergangsfenster selbst wird NICHTS erzwungen - das genau ist
        der diagnostisch dokumentierte Overlap (s. check_russell_overlap()),
        der davon unangetastet bleibt.
    Weicht die Live-Pruefung ab (z.B. weil neue Daten nach transition_end
    ueberraschend wieder die alte Quelle zeigen), kommt
    "live_reverification": "STALE_NEEDS_REVIEW" statt eines unveraenderten
    CONFIRMED-Status.

    KEINE Konsolidierung/Addition der beiden Quellen wird hier vorgenommen
    oder vorgeschlagen - das Ergebnis beschreibt nur, welche Quelle in
    welcher Zeitphase als Forschungsquelle gilt (research_policy), niemals
    eine rechnerische Kombination beider Reihen.
    """
    entry = KNOWN_EXCHANGE_MIGRATIONS.get(instrument_id)
    if not entry:
        return {"applicable": False}

    from_ex = entry["from_exchange"]
    to_ex = entry["to_exchange"]
    t_start = entry["transition_start"]
    t_end = entry["transition_end"]

    before_ok = True
    after_ok = True
    for r in rows:
        d = _date_only(r.get("report_date_as_yyyy_mm_dd"))
        name = (r.get("market_and_exchange_names") or "").upper()
        if not d:
            continue
        if d < t_start and from_ex.upper() not in name:
            before_ok = False
        if d > t_end and to_ex.upper() not in name and "CHICAGO" not in name:
            after_ok = False

    live_status = "CONFIRMED" if (before_ok and after_ok) else "STALE_NEEDS_REVIEW"

    return {
        "applicable": True,
        "migration": {
            "status": entry["status"],
            "live_reverification": live_status,
            "from_exchange": from_ex,
            "to_exchange": to_ex,
            "transition_start": t_start,
            "transition_end": t_end,
            "research_policy": entry["research_policy"],
            "evidence": entry["evidence"],
            "verified_at": entry["verified_at"],
            "note": (
                "research_policy beschreibt NUR, welche Quelle je Zeitphase "
                "als Forschungsquelle gilt - keine Addition/Konsolidierung "
                "von ICE+CME. Die 43 Overlap-Wochen bleiben zusaetzlich und "
                "unveraendert in russell_overlap dokumentiert."
            ),
        },
    }
...
def _classify_anomaly(field: str) -> tuple:
    """
    Trennt Befund und Ursache (Axel-Vorgabe): ein Feld-Gap wird NICHT
    automatisch als CFTC-Unterdrueckung erklaert, sondern als
    REVIEW_REQUIRED markiert, wenn es zum bekannten Muster passt.
    ...
    traders_*-Felder (20.09.2026, verifiziert per Claude-in-Chrome direkt
    gegen die CFTC-eigene TFF-Dokumentation, nicht nur die allgemeine COT-
    FAQ): der Unterdrueckungs-MECHANISMUS selbst ist jetzt CFTC-dokumentiert
    und TFF-spezifisch bestaetigt (Quelle unten), deshalb confidence=HIGH
    statt MEDIUM. Das bestaetigt aber NICHT automatisch, dass JEDE einzelne
    von UIQ gefundene traders_*-Luecke tatsaechlich durch diesen Mechanismus
    verursacht wurde - Existenz der Regel und Ursache eines KONKRETEN
    Einzelfunds bleiben unterschiedliche Aussagen. Klassifikation bleibt
    deshalb REVIEW_REQUIRED, nicht PASS.

    Zitat (TFF Explanatory Notes, Abschnitt "Numbers of Traders"):
    "In order to preserve the confidentiality of traders, for any given
    commodity where a specific category has fewer than four active traders,
    the size of the relevant positions will be provided but the trader
    count will not be (specifically, a "." will appear for trader counts
    of fewer than four traders)."
    Quelle: CFTC, Traders in Financial Futures Explanatory Notes,
    ...
    if field.startswith("traders_"):
        return "REVIEW_REQUIRED", {
            "mechanism": (
                "CFTC confidentiality suppression rule for TFF: fuer eine "
                "gegebene Kategorie mit weniger als vier aktiven Tradern "
                "wird die Positionsgroesse veroeffentlicht, der Trader-Count "
                "aber unterdrueckt (Platzhalter statt Zahl)."
            ),
            "confidence": "HIGH",
            "source": 'CFTC, "Traders in Financial Futures Explanatory Notes", Abschnitt "Numbers of Traders"',
            "source_url": "https://www.cftc.gov/sites/default/files/idc/groups/public/@commitmentsoftraders/documents/file/tfmexplanatorynotes.pdf",
            "note": (
                "Mechanismus ist TFF-spezifisch CFTC-dokumentiert (nicht nur "
                "durch die allgemeine COT-FAQ). Bestaetigt NICHT automatisch "
                "jeden einzelnen konkreten traders_*-Befund in diesem Report."
            ),
        }
...
def check_schema_stability(rows: list) -> dict:
    ...
    for field, dates_present in fields_seen_at.items():
        ...
        if missing_in_range and field not in ("historical_source_overlap",):
            ...
            if unexplained:
                classification, cause = _classify_anomaly(field)
                anomalies.append({
                    "field": field,
...
def build_report(base_dir: str = NORMALIZED_DIR) -> dict:
    ...
    for instrument_id in instrument_ids:
        ...
        x = check_exchange_migration(rows, instrument_id)
        if x.get("applicable"):
            checks["exchange_migration"][instrument_id] = x
            if x["migration"]["live_reverification"] == "STALE_NEEDS_REVIEW":
                statuses.append("WARN")
    ...
    # Russell-Sonderfall (20.09.2026): der Overlap blockiert full_universe_
    # research NICHT MEHR allein, sobald die zugehoerige Exchange-Migration
    # als CONFIRMED dokumentiert und live-reverifiziert ist - der Overlap
    # ist dann ein erklaertes historisches Ereignis, keine offene Frage
    # mehr. Bleibt die Migration aus irgendeinem Grund unbestaetigt (kein
    # Registry-Eintrag, oder STALE_NEEDS_REVIEW), blockiert der Overlap
    # weiterhin wie zuvor - das ist die bewusste Rueckfallposition.
    russell_migration_confirmed = (
        "RUSSELL2000" in checks["exchange_migration"]
        and checks["exchange_migration"]["RUSSELL2000"]["migration"]["status"] == "CONFIRMED"
        and checks["exchange_migration"]["RUSSELL2000"]["migration"]["live_reverification"] == "CONFIRMED"
    )
    russell_pending = (
        "RUSSELL2000" in checks["russell_overlap"]
        and checks["russell_overlap"]["RUSSELL2000"].get("applicable")
        and not russell_migration_confirmed
    )
    ...
    for instrument_id, migration_check in checks["exchange_migration"].items():
        m = migration_check["migration"]
        note = instrument_notes.setdefault(instrument_id, {})
        note["migration_status"] = m["status"]
        note["migration_live_reverification"] = m["live_reverification"]
        note["research_policy"] = m["research_policy"]
        note["transition_start"] = m["transition_start"]
        note["transition_end"] = m["transition_end"]
    ...
    if russell_pending:
        reasons.append("Russell ICE/CME Overlap: Research Decision weiterhin Pending (Migration nicht bestaetigt/live-reverifiziert) - keine Konsolidierung getroffen, full_universe_research bleibt deshalb false")
    if russell_migration_confirmed:
        reasons.append("RUSSELL2000: Exchange-Migration ICE→CME CONFIRMED (CME Group Primaerquelle + interne Net-Position-Evidenz), Uebergangsfenster 2017-07-01 bis 2018-06-05 RESEARCH_ONLY - s. instrument_notes.research_policy; blockiert full_universe_research nicht mehr")
    if instrument_notes:
        for inst, note in instrument_notes.items():
            if "stable_series_start" in note:
            ...
            # Migrations-Hinweise sind bereits oben ueber russell_migration_confirmed
            # in reasons aufgenommen - hier keine Dopplung noetig.
    if not reasons:
