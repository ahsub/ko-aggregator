...
# UnderlyingIQ — Strategiedokument v1.12
**Stand: 15.07.2026 | Status: Arbeitsgrundlage | Autor: Dr. Axel Hildebrand mit Claude**
...
E. **Alpha Desk konsolidieren** — Erste Zeile: kurze textliche Erklärung Zweck des Tabs. Beim Öffnen zunächst nur Master-Shortlist zeigen. Enrichment nur auf explizite User-Aufforderung mit Rückfrage. „KI-basierte Markt-Einschätzung"-Button liefert vielversprechendste Ticker × vielversprechendste Strategie. Strategie-Nerds: separater Weg für favorisierte Strategie mit eigener Listen-Darstellung. Marktzustands-Bezeichnung („Bull-Quiet") muss aus Briefing/Makro herleitbar oder erklärt sein. Refresh-Button rechts entweder erklären oder entfernen. **✅ Umgesetzt (15.07.2026, axel-scanner v344):** Zweck-Erklärung ergänzt; Regime-Code wird jetzt als menschenlesbares Label angezeigt (Rohcode nur noch als Tooltip); Refresh-Button war bereits in einer früheren Session entfernt, verifiziert. **Punkt "Beim Öffnen zunächst nur Master-Shortlist zeigen" REVIDIERT (Axel-Korrektur, 15.07.2026):** ursprünglich in v343 als Einklapp-Mechanismus der Leaderboard-Sektion umgesetzt, dann in v344 zurückgebaut — die Leaderboard-Leiste mit den strategiespezifischen KI-Buttons hat bereits sehr gute Ergebnisse gezeigt und soll wie zuvor immer sichtbar bleiben, dieser Spec-Punkt gilt damit als nicht (mehr) verfolgt. **Enrichment-Rückfrage:** bereits strukturell erfüllt — KI-Anreicherung passiert serverseitig im täglichen Aggregator-Lauf, es gibt clientseitig keine separate Enrichment-Aktion, die versehentlich ausgelöst werden könnte. **Neu gebaut:** übergreifender "🎯 Beste Chance über alle Strategien finden"-Button (durchsucht alle Leaderboards nach dem höchsten Score, unabhängig von der Strategie, spart das Durchklicken aller 10 Tabs) — Pareto-Abwägung (Claude, 15.07.2026): hoher Nutzen für Cold-Start-Problem neuer Nutzer, geringer Bauaufwand (alle Leaderboards haben bereits Scores); kommt ZUSÄTZLICH zur immer sichtbaren Leaderboard-Leiste, ersetzt sie nicht. **Zurückgestellt (Pareto-Abwägung, Axel + Claude, 15.07.2026):** "Strategie-Nerds"-Punkt (favorisierte Strategie mit eigener Speicherung + Listen-Darstellung) — bedient wiederkehrende Power-User mit bereits etablierter Präferenz, nicht die externen Beta-Neulinge, um die es in Phase 0.5 primär geht; unklarer Aufwand ohne genauere Spezifikation von "eigene Listen-Darstellung". Bestehende Tab-Navigation zwischen den 10 Leaderboards bleibt vorerst die Lösung für diese Zielgruppe.

F. **Options Desk als vollwertiger Modus** — Klick öffnet eigenständigen Ansichts-Modus, der Master-Shortlist verdrängt (statt versteckt darunter). KI-enriched Top-Options-Ticker prominent, KI-Einschätzung-Button liefert Underlying × Options-Strategie. Analog zu Alpha Desk: separater Weg für favorisierte Options-Strategien (Wheel, CSP/CC, Weekly Options) mit eigener Listen-Darstellung. **✅ Umgesetzt (15.07.2026, axel-scanner v345, ko-aggregator v5.10.0):** (1) Strategie-Leaderboard-Leiste (reine Equity-Strategien) blieb bisher auch im Options-/Value-Modus sichtbar — jetzt nur noch im Shortlist-Modus, echter Modus-Wechsel. (2) Options-Watchlist bekommt jetzt echte serverseitige KI-Anreicherung (analog zur Master-Shortlist, Top-15: Strike/DTE/Delta/Prämie-Vorschlag), Client zeigt das prominent per Badge + Detail-Box + Gesamt-Status an — vorher hätte eine reine Client-Badge ohne echte Server-Daten irregeführt. (3) Neuer übergreifender "Beste Underlying × Strategie-Kombination"-Button — deterministische Ticker-Auswahl (höchster Gesamt-Score), Strategie-Wahl bewusst an die KI delegiert statt algorithmisch geraten (4 Score-Felder CSP/CC/Spread/Gesamt bilden nicht 1:1 auf eine Strategie ab). **Zurückgestellt (Punkt 4, konsistent zur analogen Entscheidung in Arbeitspaket E):** separater Weg für favorisierte Options-Strategien mit eigener Listen-Darstellung — bedient wiederkehrende Power-User, nicht die externen Beta-Neulinge der Phase 0.5.

G. **Scanner-Tab Neuordnung** — Marktstimmungs-Anzeige konsistent zur Alpha-Desk-Anzeige (heute widersprüchlich). Strategie-Button-Leiste entweder funktional oder als Info-Chips markiert. „Aufklappen/Zuklappen"-Buttons über die Tickerkarten. „Scan" vs. „Live" per Mouse-over erklärt. Handelsplatz-Auswahl mit Auto-Sinnvoll-Watchlists (DE → DAX/TecDAX/MDAX, US → S&P500/NASDAQ100), bei fehlender Watchlist als Popup-Empfehlung + Caching. Erklärungs-Tooltips für „KI-Analyse Watchlist Top-Performer" mit EIC-Modus-Verhalten. **✅ Vollständig abgeschlossen (15.07.2026, axel-scanner v346/v347, ko-modules@c52bac1):** Marktstimmungs-Anzeige vereinheitlicht — wichtiger Fund dabei: Scanner zeigte die MSE-Regime (client, VVIX/SKEW-basiert, Grundlage der Strategie-Ampel seit MCM), Alpha Desk die Aggregator-Regime (server, Markov-basiert, steuert Leaderboard-Ticker-Auswahl) — zwei legitime, aber unterschiedliche Klassifikationen mit identischen Badges. Axel-Entscheidung: nur die Anzeige vereinheitlichen (beide zeigen jetzt die MSE-Regime), die Aggregator-interne Stock-Selection-Logik bewusst nicht anfassen (Risikovermeidung). Strategie-Button-Leiste, Aufklappen/Zuklappen-Beschriftung und „Live"-Button-Erklärung waren bereits durch frühere Arbeitspakete (D, C) bzw. eine frühere Session erledigt/gegenstandslos. Neu gebaut: Auto-Sinnvoll-Watchlist-Empfehlung (Erst-Nutzer-Toast + ⭐-Markierung) mit Caching der letzten Wahl je Handelsplatz; EIC-Modus-Tooltips für den Watchlist-Top-Performer-KI-Button (verifiziert gegen den echten ko-ai-Worker-Code: Public vs. Expert System-Prompt-Unterschied).

H. **„Wolf-im-Schafspelz"-Modus (Investor-friendly)** — Non-Nerd-Ansicht im Admin/User-Bereich umschaltbar, reduziert Feature-Vielfalt auf das für den Anlage-Alltag Notwendige. Ziel: Einschüchterung durch Feature-Dichte abmildern, ohne den Expert-Modus zu verlieren. Erste Konzept-Skizze in Phase 0.5, Umsetzung im Detail möglicherweise erst in Phase-1-Iteration. **📝 Konzept-Skizze (15.07.2026, wie von der Spec vorgesehen — bewusst keine Implementierung in Phase 0.5):**

*Zielgruppe:* Ein Investor, der UnderlyingIQ zur Unterstützung von Anlageentscheidungen nutzt, aber kein Interesse an technischen Details (RSI, ATR, Markov-Regime, VCP-Setup-Erkennung, Fibo-Confluence, Squeeze-Risk-Scores etc.) hat — will schlicht wissen: "Was soll ich heute tun?"

*Kernidee — globaler Toggle "Investor-Modus":* Analog zum bestehenden "Kurz und bündig"-Toggle (localStorage `ko_simple_lang`) ein neuer Toggle `ko_investor_mode`, der eine `body.investor-mode`-CSS-Klasse setzt. Alle als "Nerd-only" markierten UI-Elemente bekommen eine gemeinsame CSS-Klasse (`.nerd-only`), die im Investor-Modus per `body.investor-mode .nerd-only { display: none; }` ausgeblendet wird — technisch simpel und risikoarm (reine CSS-Sichtbarkeitssteuerung, keine Logik-Änderung, jederzeit reversibel).

*Was ausgeblendet würde (Grobskizze, nicht abschließend):*
- Scanner-Tab: technische Chip-Reihen (RSI/ATR/MACD/OBV-Details) — zeigt nur Grade + KI-Einschätzung
- Alpha Desk: die 10 Strategie-Leaderboard-Tabs komplett — zeigt nur Master-Shortlist + den "Beste Chance"-Button (Arbeitspaket E)
- Options Desk: komplett ausgeblendet (zu komplex fürs Anlage-Alltag-Ziel) oder auf eine einzige, stark vereinfachte Ansicht reduziert
- Value-Scanner-Detailmetriken (KGV/PEG/ROE-Rohwerte) — nur Gesamt-Einschätzung sichtbar
- Admin-Bereich: Backup-Export/Import, EIC-PIN-Verwaltung, API-Key-Rohansicht — bleiben ausschließlich im Nicht-Investor-Modus sichtbar
- VCP-/Fibo-/Squeeze-Risk-Badges auf allen Ticker-Karten

*Was NICHT betroffen wäre:* Home-Panel (bereits der einfachste Einstiegspunkt, siehe Arbeitspaket A), Morning Briefing (bereits als KI-Fließtext konsumierbar), regulatorische Hinweise (WpHG-Disclaimer bleiben immer sichtbar, unabhängig vom Modus).

*Warum keine Implementierung in Phase 0.5:* Das Markieren aller betroffenen UI-Elemente mit `.nerd-only` wäre ein sehr großer, invasiver Eingriff quer durch das gesamte Interface (geschätzt 50-100+ einzelne Elemente über alle Panels hinweg), der eine sorgfältige Einzelfall-Entscheidung braucht ("ist X Alltags-relevant oder Nerd-Detail?") — das würde selbst mehrere Tage/eine eigene Session brauchen und sprengt den Rahmen der aktuellen, primär auf schnelle Bedienungs-Klarheit ausgelegten Arbeitspakete A-G. Passt strukturell besser als eigenständiger Sprint zu Beginn von Phase 1, nach Track-Record-Start, wenn auch reales Nutzerfeedback zur tatsächlichen Feature-Dichte-Wahrnehmung vorliegt (aktuell rein spekulativ, ohne Beta-Tester-Daten).

*Offene Entscheidung für Phase 1:* Soll der Investor-Modus der DEFAULT für neue Nutzer sein (mit Opt-in zum Experten-Modus), oder umgekehrt (Experten-Modus Default, Investor-Modus als Opt-in)? Berührt dieselbe Grundsatzfrage wie der "Kurz und bündig als Standard"-Fix in Arbeitspaket C — vermutlich sollte auch hier die einfachere Ansicht der Default sein.

I. **Fehler-Kommunikation KI-Aufrufe (akut, aus Beobachtung 08.07.2026)** — Beim wiederholten Rate-Limit-Fehler („⚠️ Fetch-Fehler: Rate-Limit erreicht — bitte kurz warten") passiert im UI danach *nichts weiter*: keine automatische Wiederholung, keine Wartezeit-Angabe, kein Retry-Button, kein Cache-Fallback auf vorherige Antwort, keine Zustands-Klarheit. Der Nutzer weiß auch nicht, ob es sich um ein temporäres RPM/TPM-Limit (60 s warten) oder ein aufgebrauchtes Tageskontingent handelt (bis morgen warten) oder eine leere Credit-Balance (aufladen). Arbeitspaket: (1) Unterscheidbare Meldungen je 429-Grund, (2) sichtbarer Retry-Button mit Countdown bei transienten Limits, (3) Cache-Fallback auf zuletzt gespeicherte KI-Einschätzung für denselben Ticker mit Alters-Anzeige, (4) klarer Zustands-Wechsel im UI (nicht nur Toast). Kleiner Aufwand, hoher UX-Effekt. Verweist strukturell auf Phase 1 (Skalierungs-Konzept, siehe dort). **✅ Vollständig abgeschlossen (15.07.2026, axel-scanner v348):** Punkte 1+4 waren bereits in einer früheren Session (v245) als zentrale `showKiError()`-Funktion gebaut — diese Beobachtung war zum Zeitpunkt der Spec-Formulierung noch aktuell, danach aber bereits behoben (Spec-Text hier historisch belassen, siehe Umsetzungsvermerk). Neu ergänzt: Countdown am Retry-Button (20s) bei transienten Rate-Limits, Cache-Fallback auf die letzte KI-Einschätzung desselben Tickers mit Alters-Anzeige (rollierender Cache, max. 30 Ticker). Zusatzfund: die Einzeltitel-KI-Analyse im Deep-Dive nutzte die zentrale Fehlerbehandlung bisher gar nicht (nur rohe Fehlermeldung) — jetzt konsistent umgestellt.

J. **Mobile-Tauglichkeit** *(15.07.2026, Axel-Initiative — "von vielen potenziellen Usern lautstark gewünscht")* — Eigenständiges Arbeitspaket, nicht als Nebenprodukt anderer Pakete (insb. C) mitgelöst. **Ist-Zustand-Befund (15.07.2026):** `viewport`-Meta-Tag korrekt gesetzt, genau EIN echter Breakpoint (`@media(max-width:700px)`) vorhanden — verwandelt die Sidebar-Navigation in eine Bottom-Tab-Leiste, funktioniert. Deckt aber NUR die Navigations-Hülle ab, NICHT: Daten-Tabellen/Grids (viele feste `grid-template-columns` unabhängig von Bildschirmbreite), Touch-Target-Größen (viele Buttons mit `padding:2px 4px` o.ä. — unter gängigen Empfehlungen von ~44×44px), Modals/Dropdowns auf schmalen Screens. **Wichtiger Zusammenhang mit Arbeitspaket C:** Icon-only-Buttons ohne sichtbaren Text (Tooltip-Only-Beschriftung) sind auf Mobile grundsätzlich unentdeckbar, da es dort kein Hover gibt — die C-Fixes (sichtbare Text-Labels statt reiner Icons) kommen Mobile-Nutzern dadurch zugute, ohne mobile-spezifischen Mehraufwand; werden im Zuge von C bereits mit Blick auf Touch-Freundlichkeit umgesetzt. **Arbeitspaket-Scope (noch nicht sprint-committed):** (1) Systematischer Audit welche Kernansichten (Alpha Desk, Scanner-Tabellen, Options Desk) auf schmalen Viewports tatsächlich brechen; (2) Touch-Target-Audit + Nachbesserung der kleinsten Buttons/Chips; (3) Entscheidung über zusätzliche Breakpoints (Tablet-Zwischenstufe?); (4) Modal-/Dropdown-Verhalten auf schmalen Screens pruefen. **Bewusste Entscheidung zur Reihenfolge (Claude + Axel, 15.07.2026):** kein striktes "erst 100% Desktop, dann Mobile bei Null anfangen" — aber auch kein hastiges Mitlösen. Aktuelle Text-/Beschriftungs-Arbeiten (C) laufen normal weiter, Mobile-Audit als eigene, dedizierte Session-Aufgabe vorgemerkt.
...
| 15.07.2026 | **Phase 0.5 Arbeitspaket J neu: Mobile-Tauglichkeit** (Axel-Initiative, "von vielen potenziellen Usern lautstark gewünscht") — eigenständiges Paket, nicht als Nebenprodukt von AP C mitgelöst. Ist-Zustand-Befund: `viewport`-Meta korrekt, ein funktionierender Breakpoint (`max-width:700px`, Sidebar→Bottom-Tab-Leiste), deckt aber nur die Navigations-Hülle ab — Daten-Tabellen/Grids, Touch-Target-Größen und Modal-Verhalten auf schmalen Screens bislang ungeprüft. Zusammenhang mit AP C erkannt: Icon-only-Buttons mit reinem Hover-Tooltip sind auf Mobile grundsätzlich unentdeckbar (kein Hover) — AP-C-Fixes (sichtbare Text-Labels) kommen Mobile ohne mobile-spezifischen Mehraufwand zugute. Reihenfolge-Entscheidung: kein striktes Desktop-zuerst-dann-Mobile-bei-Null, aber auch kein hastiges Mitlösen — AP C läuft normal weiter (mit Blick auf Touch-Freundlichkeit), Mobile-Vollaudit als eigene, dedizierte Session vorgemerkt. Gleichzeitig AP C erste Runde umgesetzt (axel-scanner v339): 5 TradingView-Icon-Buttons + 4 weitere echt mehrdeutige Icon-Buttons (Aufklappen/Zuklappen, Backup Export/Import, Zwischenablage einfügen, "Leeren") bekommen sichtbare Text-Labels; 3 mit universell verständlichen Icon-Konventionen (Auge, Papierkorb, Stern) bewusst unverändert gelassen. |
| 15.07.2026 | **Phase 0.5 Arbeitspakete C+D+E abgeschlossen** (axel-scanner v340-v343, ko-modules@c72b7bf). AP C: KI-Analyse-Umbenennung + echter Default-Bug gefunden (neue Nutzer bekamen faelschlich die technische Zahlen-Ansicht statt "Kurz und bündig" trotz Spec-Vorgabe "als Standard-Ansicht") + Modell-Herkunft aus Kunden-UI entfernt + min-Label + Handelsplatz-Wording. AP D: Strategie-Ampel-Chips — 7 mit eindeutiger Leaderboard-Entsprechung bekommen echte Klick-Funktion (Navigation zum passenden Alpha-Desk-Tab), 4 ohne eindeutige Entsprechung verlieren die Button-Optik (keine geratenen Mappings); alle 5 Regime-Action-Texte bekommen "Gesamteinschätzung: "-Praefix zur Kontext-Klaerung. AP E: Zweck-Erklaerung ergaenzt, Leaderboard-Sektion standardmaessig eingeklappt (nur Master-Shortlist beim Oeffnen), Regime-Code als menschenlesbares Label statt Rohcode, neuer uebergreifender "Beste Chance über alle Strategien"-Button (Pareto-Entscheidung: gebaut, da hoher Nutzen/geringer Aufwand fuer Cold-Start-Problem), "Strategie-Nerds"-Feature (favorisierte Strategie) zurueckgestellt (bedient wiederkehrende Power-User, nicht die Beta-Neulinge, um die es in Phase 0.5 primaer geht). |
| 15.07.2026 | **Phase 0.5 vollstaendig abgeschlossen — alle Arbeitspakete A-J bearbeitet** (axel-scanner v344-v348, ko-modules@c52bac1). AP F: Options Desk als vollwertiger Modus (Leaderboard-Leiste nur noch im Shortlist-Modus, echte serverseitige KI-Anreicherung fuer Options-Watchlist neu gebaut statt vorgetaeuschter Client-Badge, uebergreifender Beste-Kombination-Button). AP G: Marktstimmungs-Anzeige vereinheitlicht (wichtiger Fund: Scanner zeigte MSE-Regime, Alpha Desk Aggregator-Regime — zwei legitime, aber unterschiedliche Systeme; Axel-Entscheidung: nur Anzeige vereinheitlichen, Stock-Selection-Logik unangetastet lassen), Auto-Sinnvoll-Watchlist-Empfehlung + Caching, EIC-Modus-Tooltips. AP H: Konzept-Skizze "Wolf-im-Schafspelz"-Investor-Modus dokumentiert (wie von der Spec vorgesehen, keine Implementierung in Phase 0.5). AP I: Retry-Countdown + Pro-Ticker-Cache-Fallback bei KI-Fehlern ergaenzt (Basis-Fehlerbehandlung war teils schon aus einer frueheren Session vorhanden). Ein Zwischenschritt in AP E (Leaderboard-Einklappen) wurde nach Axel-Feedback wieder zurueckgebaut, da die Leiste zusaetzlich zum neuen Button erhalten bleiben sollte, nicht ersetzt. |
